...
func (p *ActorAccount) registerDevAccount(req *pb.DevRegister) int32 {
	...
	mysql := p.App().Find("db_center_component").(*db.Component).DB

	return db.AccountRegister(mysql, accountName, password, req.Ip)
}
...
func (p *ActorAccount) getDevAccount(req *pb.DevRegister) (*pb.Int64, int32) {
	...
	devAccount, _ := db.AccountWithName(mysql, accountName)
...
func (p *ActorAccount) getUID(req *pb.User) (*pb.Int64, int32) {
	...
	// 获取组件
	rdb := p.App().Find("db_center_component").(*db.Component).RDB

	mysql := p.App().Find("db_center_component").(*db.Component).DB

	uid, ok := db.BindUIDInDB(rdb, mysql, req.SdkId, req.Pid, req.OpenId)
	if uid == 0 || ok == false {
